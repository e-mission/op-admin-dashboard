...
def query_uuids(start_date: str, end_date: str, tz: str):
    ...
    logging.debug("Utils - query_uuids Stage 1: Querying the UUID DB for (no date range)")
    ...
    logging.debug(f"Utils - query_uuids Stage 2: Time taken for fetching UUID entries: {time.time() - entries_start_time:.4f} seconds")
    ...
    end_time = time.time()  # End timing
    execution_time = end_time - start_time
    logging.debug(f'Utils - query_uuids Stage 3: Total time taken for query_uuids: {execution_time:.4f} seconds')
...
def query_confirmed_trips(start_date: str, end_date: str, tz: str):
    ...
    logging.debug("Utils - query_confirmed_trips Stage 1: Converting date range to timestamps")
    ...
    logging.debug(f"Utils - query_confirmed_trips Stage 2: Time taken for get_aggregate_time_series: {time.time() - ts_start_time:.4f} seconds")
    ...
    logging.debug(f"Utils - query_confirmed_trips Stage 3: Time taken for get_data_df: {time.time() - entries_start_time:.4f} seconds")

    user_input_cols = []

    logging.debug("Utils - query_confirmed_trips Stage 4: Before filtering, df columns are %s" % df.columns)
    if not df.empty:
        ...
        logging.debug("Utils - query_confirmed_trips Stage 5: Rename mapping is %s" % rename_mapping)
        df.rename(columns=rename_mapping, inplace=True)
        logging.debug("Utils - query_confirmed_trips Stage 6: After renaming columns, they are %s" % df.columns)
        ...
        logging.debug(f"Utils - query_confirmed_trips Stage 7: Time taken for copying coordinates: {time.time() - coords_start_time:.4f} seconds")
        ...
        if 'ble_sensed_summary' in df.columns:
            ...
            logging.debug("Utils - query_confirmed_trips Stage 8: No BLE support found, ignoring...")

        logging.debug(f"Utils - query_confirmed_trips Stage 9: Time taken for mode processing: {time.time() - mode_processing_start_time:.4f} seconds")
        ...
        logging.debug(f"Utils - query_confirmed_trips Stage 10: Before filtering user input {user_input_df.columns=}")
        user_input_cols = [c for c in user_input_df.columns
            if "metadata" not in c and
               ...
               "_id" not in c]
        logging.debug(f"Utils - query_confirmed_trips Stage 11: After filtering user input {user_input_cols=}")

        combined_col_list = list(perm_utils.get_all_trip_columns()) + user_input_cols
        logging.debug(f"Utils - query_confirmed_trips Stage 12: Combined list {combined_col_list=}")
        columns = [col for col in combined_col_list if col in df.columns]
        df = df[columns]
        logging.debug(f"Utils - query_confirmed_trips Stage 13: After filtering against the combined list {df.columns=}")
    ...
    end_time = time.time()  # End timing
    execution_time = end_time - start_time
    logging.debug(f'Utils - query_confirmed_trips Stage 14: Total time taken for query_confirmed_trips: {execution_time:.4f} seconds')
...
def query_demographics():
    start_time = time.time()  # Start timer for the entire function
    logging.debug("Utils - query_demographics Stage 1: Querying the demographics for (no date range)")

    ts_start_time = time.time()  # Start timing for time series object retrieval
    ts = esta.TimeSeries.get_aggregate_time_series()
    logging.debug(f"Utils - query_demographics Stage 2: Time taken for get_aggregate_time_series: {time.time() - ts_start_time:.4f} seconds")

    entries_start_time = time.time()  # Start timing for finding entries
    entries = ts.find_entries(["manual/demographic_survey"])
    data = list(entries)
    logging.debug(f"Utils - query_demographics Stage 3: Time taken for find_entries: {time.time() - entries_start_time:.4f} seconds")
    ...
    for key, json_object in available_key.items():
        ...
        dataframes[key] = df
        logging.debug(f"Utils - query_demographics Stage 4: Time taken for json_normalize for key '{key}': {time.time() - df_normalize_start_time:.4f} seconds")
        ...
        if not df.empty:
            ...
            for col in constants.BINARY_DEMOGRAPHICS_COLS:
                if col in df.columns:
                    df[col] = df[col].apply(str)
            logging.debug(f"Utils - query_demographics Stage 5: Time taken for converting binary demographic columns to strings: {time.time() - col_convert_start_time:.4f} seconds")

            # Drop metadata columns
            drop_metadata_start_time = time.time()
            columns_to_drop = [col for col in df.columns if col.startswith("metadata")]
            df.drop(columns=columns_to_drop, inplace=True)
            logging.debug(f"Utils - query_demographics Stage 6: Time taken for dropping metadata columns: {time.time() - drop_metadata_start_time:.4f} seconds")
            ...
            logging.debug(f"Utils - query_demographics Stage 7: Time taken for renaming columns: {time.time() - rename_start_time:.4f} seconds")
            ...
            logging.debug(f"Utils - query_demographics Stage 8: Time taken for dropping excluded columns: {time.time() - drop_excluded_start_time:.4f} seconds")

    total_time = time.time() - start_time  # End timer for the entire function
    logging.debug(f"Utils - query_demographics Stage 9: Total time taken for query_demographics: {total_time:.4f} seconds")
...
def query_trajectories(start_date: str, end_date: str, tz: str, key_list):
    ...
    (start_ts, end_ts) = iso_range_to_ts_range(start_date, end_date, tz)
    logging.debug(f"Utils - query_trajectories Stage 1: Time taken for iso_range_to_ts_range: {time.time() - start_time:.4f} seconds")

    # Get the time series object
    ts = esta.TimeSeries.get_aggregate_time_series()
    logging.debug(f"Utils - query_trajectories Stage 2: Time taken for get_aggregate_time_series: {time.time() - start_time:.4f} seconds")
    ...
    logging.debug(f"Utils - query_trajectories Stage 3: Time taken for find_entries: {time.time() - entries_start_time:.4f} seconds")

    # Normalize entries into a DataFrame
    df_normalize_start_time = time.time()
    df = pd.json_normalize(list(entries))
    logging.debug(f"Utils - query_trajectories Stage 4: Time taken for json_normalize: {time.time() - df_normalize_start_time:.4f} seconds")
    ...
    if not df.empty:
        ...
        for col in df.columns:
            if df[col].dtype == 'object':
                df[col] = df[col].apply(str)
        logging.debug(f"Utils - query_trajectories Stage 5: Time taken for converting object columns to strings: {time.time() - col_convert_start_time:.4f} seconds")

        # Drop metadata columns
        drop_metadata_start_time = time.time()
        columns_to_drop = [col for col in df.columns if col.startswith("metadata")]
        df.drop(columns=columns_to_drop, inplace=True)
        logging.debug(f"Utils - query_trajectories Stage 6: Time taken for dropping metadata columns: {time.time() - drop_metadata_start_time:.4f} seconds")
        ...
        for col in constants.EXCLUDED_TRAJECTORIES_COLS:
            if col in df.columns:
                df.drop(columns=[col], inplace=True)
        logging.debug(f"Utils - query_trajectories Stage 7: Time taken for dropping excluded columns: {time.time() - drop_excluded_start_time:.4f} seconds")
        ...
        logging.debug(f"Utils - query_trajectories Stage 8: Time taken for background/location processing: {time.time() - location_check_start_time:.4f} seconds")

    total_time = time.time() - start_time  # End timer for the entire function
    logging.debug(f"Utils - query_trajectories Stage 9: Total time taken for query_trajectories: {total_time:.4f} seconds")
...
def add_user_stats(user_data, batch_size=10):
    ...
    def process_user(user):
        ...
        logging.debug(f"Utils - add_user_stats Stage 1: Time taken for get_aggregate_time_series: {time.time() - ts_aggregate_start_time:.4f} seconds")
        ...
        logging.debug(f"Utils - add_user_stats Stage 2: Time taken for find_one user profile: {time.time() - profile_data_start_time:.4f} seconds")
        ...
        logging.debug(f"Utils - add_user_stats Stage 3: Time taken for counting trips: {time.time() - trips_count_start_time:.4f} seconds")
        ...
        if total_trips > 0:
            ...
            logging.debug(f"Utils - add_user_stats Stage 4: Time taken for get_time_series: {time.time() - ts_user_start_time:.4f} seconds")
            ...
            logging.debug(f"Utils - add_user_stats Stage 5: Time taken for retrieving first/last trip and last call: {time.time() - first_last_trip_start_time:.4f} seconds")

        logging.debug(f"Utils - add_user_stats Stage 6: Total time taken for process_user: {time.time() - process_user_start_time:.4f} seconds")
        return user
    ...
    def batch_process(users_batch):
        ...
        logging.debug(f"Utils - add_user_stats Stage 7: Time taken for batch processing {len(users_batch)} users: {time.time() - batch_start_time:.4f} seconds")
    ...
    total_time = time.time() - start_time  # End timer for the entire function
    logging.debug(f"Utils - add_user_stats Stage 8: Total time taken for add_user_stats: {total_time:.4f} seconds")
...
def query_segments_crossing_endpoints(poly_region_start, poly_region_end, start_date: str, end_date: str, tz: str, excluded_uuids: list[str]):
    ...
    (start_ts, end_ts) = iso_range_to_ts_range(start_date, end_date, tz)
    logging.debug(f"Utils - query_segments_crossing_endpoints Stage 1: Converted dates to timestamps: {start_ts}, {end_ts}")
    ...
    logging.debug(f"Utils - query_segments_crossing_endpoints Stage 2: Found {len(locs_matching_start)} locations matching the start region in {locs_matching_start_time:.4f} seconds.")
    ...
    logging.debug(f"Utils - query_segments_crossing_endpoints Stage 3: Found {len(locs_matching_end)} locations matching the end region in {locs_matching_end_time:.4f} seconds.")
    ...
    logging.debug(f"Utils - query_segments_crossing_endpoints Stage 4: Merged DataFrames in {merge_time:.4f} seconds.")
    ...
    logging.debug(f"Utils - query_segments_crossing_endpoints Stage 5: Number of unique users seen: {number_user_seen}")
    ...
    if perm_utils.permissions.get("segment_trip_time_min_users", 0) <= number_user_seen:
        ...
        logging.debug(f"Utils - query_segments_crossing_endpoints Stage 6: Total time taken for query_segments_crossing_endpoints: {total_time:.4f} seconds.")
        return filtered
    
    logging.debug("Utils - query_segments_crossing_endpoints Stage 7: Minimum user requirement not met, returning empty DataFrame.")
