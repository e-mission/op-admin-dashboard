...
def query_confirmed_trips(start_date: str, end_date: str, tz: str):
    ...
    if not df.empty:
        ...
        rename_mapping = dict(
            zip([c.replace("data.", "") for c in rename_cols], rename_cols)
        )
        ...
        df["data.start_loc.coordinates"] = df["start_loc"].apply(
        ...
        df["data.primary_sensed_mode"] = df.cleaned_section_summary.apply(
            get_max_mode_from_summary
        )
        df["data.primary_predicted_mode"] = df.inferred_section_summary.apply(
            get_max_mode_from_summary
        )
        if "ble_sensed_summary" in df.columns:
            df["data.primary_ble_sensed_mode"] = df.ble_sensed_summary.apply(
                get_max_mode_from_summary
            )
        else:
        ...
        user_input_cols = [
            c
            for c in user_input_df.columns
            if "metadata" not in c
            and "xmlns" not in c
            and "local_dt" not in c
            and "xmlResponse" not in c
            and "_id" not in c
        ]
        ...
        use_imperial = perm_utils.config.get(
            "display_config", {"use_imperial": False}
        ).get("use_imperial", False)
        ...
        df["data.duration"] = df["data.duration"].apply(
    ...
    for key, df in dataframes.items():
        if not df.empty:
            ...
            df.columns = [
                (
                    col.rsplit(".", 1)[-1]
                    if col.startswith("data.jsonDocResponse.")
                    else col
                )
...
def query_trajectories(start_date: str, end_date: str, tz: str):
    ...
    if not df.empty:
        ...
        df["data.mode_str"] = df["data.mode"].apply(
            lambda x: (
                ecwm.MotionTypes(x).name
                if x in set(enum.value for enum in ecwm.MotionTypes)
                else "UNKNOWN"
            )
        )
    return df
